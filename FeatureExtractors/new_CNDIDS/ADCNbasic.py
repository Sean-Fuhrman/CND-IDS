...
class ADCN():
    ...
    def getMetricLoss(self, x, winIdx):
        pseudo_labels = self.getPsuedoLabels(x)
        x = x.to(self.device)
        x = self.ADCNcnn(x)
        embeddings = self.ADCNae[winIdx].network(x)
        indices_tuple = mining_func(embeddings, psuedo_labels)
        loss = loss_func(embeddings, psuedo_labels, indices_tuple)
        return loss
    
    def metricTrainBasicAE(self, psuedo_labels, x,winIdx ,distance, reducer, loss_func, mining_func, logger):
        self.ADCNae[winIdx].network = self.ADCNae[winIdx].network.train()
        self.ADCNae[winIdx].network = self.ADCNae[winIdx].network.to(self.device)
        x = x.to(self.device)
        psuedo_labels = psuedo_labels.to(self.device)
        x = self.ADCNcnn(x)
        embeddings = self.ADCNae[winIdx].network(x)
        optimizer = torch.optim.Adam(self.ADCNae[winIdx].network.parameters(), lr=0.001)
        indices_tuple = mining_func(embeddings, psuedo_labels)
        loss = loss_func(embeddings, psuedo_labels, indices_tuple)
    ...
    def getPseudoLabels(self, x):
        return self.labeler.fit_transform(x)
    
    def fitMetric(self, x, x_enc, labeler, logger, distance, reducer, loss_func, mining_func):
        psuedo_labels = self.getPseudoLabels(x, labeler)
        logger.info("Unique Pseudo Labels: %s", np.unique(psuedo_labels))
        # if len(np.unique(psuedo_labels) == 1):
        #     return
        for idx,_ in enumerate(self.ADCNae):
            self.metricTrainBasicAE(torch.tensor(psuedo_labels), x, idx ,distance, reducer, loss_func, mining_func, logger)
        
    # def setupMetricLearning(self, x, labeler):
