...
class Met2Recon(torch.nn.Module):
    ...
    def fit(self, x, device="cuda"):
        ...
        for epoch in range(1, self.train_epochs + 1):
            ...
            for i in tqdm(range(0, len(x), batch_size)):
                ...
                loss = reconstruction_loss + metric_loss
                loss.backward()
                optimizer.step()
                losses.append(loss.item())
            logger.info("Epoch %d, Loss: %f, Normal Loss: %f, Recon Loss: %f", epoch, sum(losses)/len(losses), sum(metric_losses)/len(metric_losses), sum(reconstruction_losses)/len(reconstruction_losses))
