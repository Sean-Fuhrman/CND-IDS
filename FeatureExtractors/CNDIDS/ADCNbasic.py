...
class ADCN():
    ...
    def initialization(self, labeledData, winIdx, batchSize = 16, epoch = 100, device = torch.device('cpu')):
        ...
        if winIdx == 0:
            ...
            latentFeatures = self.trainBasicAe(latentFeatures, labeledData, evolving = self.evolving, winIdx = 0, epoch = epoch, unlabeled = False, metricLoss = False)
    ...
    def trainBasicAe(self, x, x_ori, winIdx, epoch = 1, clustering = False, evolving = True, unlabeled = True, metricLoss = True):
        ...
        for iEpoch in range(0, epoch):
            ...
            # load data
            minibatch_xTrain = x         # input with masking noise
            minibatch_x      = x               # original input
            minibatch_x_ori  = x_ori
            ...
            logger.info("Reconstruction Loss: %f", loss.item())
            
            if metricLoss:
                # metric loss
                delta = 1.0
                loss_metric = self.getMetricLoss(minibatch_x,minibatch_x_ori,winIdx)
                logger.info("Metric Loss: %f", loss_metric.item())
                loss = loss * (1 - delta) + loss_metric * delta
    ...
    def trainAE(self, x, x_ori, epoch = 1, clustering = False, evolving = True, metricLoss = True):
        ...
        for idx, _ in enumerate(self.ADCNae):
            x = self.trainBasicAe(x, x_ori, idx, epoch = epoch, clustering = clustering, evolving = evolving, metricLoss = metricLoss)

    def fit(self, x, epoch = 1, metricLoss = True):
        # train ADCN, all network structures, layer wise manner
        # x is image data with size 1 x 28 x 28
        extractedFeatures = self.trainCNN(x, epoch = epoch)
        self.trainAE(extractedFeatures, x, epoch = epoch, evolving = self.evolving, metricLoss = metricLoss)
    ...
    def getMetricLoss(self, x, x_ori, winIdx):
        pseudo_labels = self.psuedo_labels.to(self.device)
        exluded = (pseudo_labels == -1).to(self.device)
        x = x[~exluded]
        pseudo_labels = pseudo_labels[~exluded]
        embeddings = self.ADCNae[winIdx].network(x)
        indices_tuple = self.mining_func(embeddings, pseudo_labels)
        loss = self.loss_func(embeddings, pseudo_labels, indices_tuple)
        return loss
    
    # def getPseudoLabels(self):
    #     return labeler.transform(x)
    # ============================= Testing =============================
    def encode(self, x):
        with torch.no_grad():
            x = x.to(self.device)
