...
def run_VaDE(datastream, config, df):
    from FeatureExtractors.VaDE import VaDE
    import argparse
    ...
    args.hid_dim = 10
    args.cuda = True
    args.input_dim = datastream.nInput
    ...
    for i, experience in enumerate(datastream.train_experiences):
        ...
        dl = DataLoader(dataset, batch_size=800, shuffle=True)
        # convert to dataloader
        model.pre_train(dl, pre_epoch=10)

        opti=Adam(model.parameters(),lr=2e-3)
        ...
        for i in range(config['train_epochs']):
            ...
            for j, test_experience in enumerate(datastream.test_experiences):
                ...
                x_sample, y_sample = datastream.get_KMeans_subset(i)
        
                x_sample_clusters = model.predict(x_sample)
                ...
                x_labels = np.array([cluster_to_label[cluster] for cluster in pred_cluster])
                

                f1 = metrics.f1_score(Y_test, x_labels)
                acc = (x_labels == Y_test).sum() / len(Y_test)
                
                logger.info("Metrics: %s", config['metrics'])
                logger.info("Metric Scores %s", [f1, acc.item()])

                df.loc[len(df.index)] = [i, j] + [f1, acc.item()]
        return df
